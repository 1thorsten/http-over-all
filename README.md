...
Integrates a proxy which delivers the newest content always.
Access restriction (basic auth, file based) is possible.
...
- support WEBDAV for the provided resources and allow the following methods DELETE MKCOL COPY MOVE PROPFIND OPTIONS
    - Allow to change files where it is possible and desired
...
| ENV-Variable | Description | required |
| -------------| ------------| ---------|
| DOCKER_[COUNT]_IMAGE | e.g. docker-repo.softconex.de/softconex/sce-scripts-aerownapi | x |
| DOCKER_[COUNT]_LOGIN | e.g. docker login docker-repo.softconex.de -u sce -p conexit | - |
| DOCKER_[COUNT]_TAG | tag (default: latest) | - |
| DOCKER_[COUNT]_METHOD | method to get sync the data (default: TAR) | - |
...
http-over-all has an simple periodic update mechanism which addresses
